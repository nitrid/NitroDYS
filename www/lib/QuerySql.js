var QuerySql = 
{
    ...
    RafTanimlariGetir : 
    {
        query : "SELECT KODU AS KODU,CONVERT(NVARCHAR(10),TIP) AS TIP,CONVERT(NVARCHAR(10),KAT) AS KAT,SIRA AS SIRA,STOK AS STOK,EN AS EN,BOY AS BOY,MIKTAR AS MIKTAR,YUKSEKLIK AS YUKSEKLIK,KATEGORI AS KATEGORI FROM RAFLAR WHERE KODU = @KODU",
        param : ['KODU'],
        type : ['string|25']
    },
    RafTanimlariSil :
    ...
    PaletTanimlariGetir : 
    {
        query : "SELECT PALET AS KODU,STOK AS STOK,PARTI AS PARTI,FORMAT(SKT,'dd.MM.yyyy') AS SKT,MIKTAR AS MIKTAR,RAF AS RAF FROM [PALET_VIEW_01] WHERE PALET = @KODU",
        param : ['KODU'],
        type : ['string|25']
    },
    StokTanimlariKaydet : 
    ...
    {
        query : "SELECT * " +
        " ,(SELECT ADI FROM CARILER WHERE KODU = EMIR_HAREKETLERI.CIKIS) AS CARIADI  FROM EMIR_HAREKETLERI WHERE TIP = 0 AND CINS = 3 AND SERI = @SERI AND SIRA = @SIRA",
