    ...
    },
    {
        ...
        insert : 
        {
            query : "DECLARE @UIDTABLE table([sth_Guid] [uniqueidentifier]) " +
                    ...
                    ",(SELECT ISNULL(MAX(sth_satirno),-1) + 1 AS SATIRNO FROM STOK_HAREKETLERI WHERE sth_evrakno_seri = @SERI AND sth_evrakno_sira = @SIRA )	--<sip_satirno, int,> \n" +
            ...
            param : ['TARIH:string|10','SERI:string|10','SIRA:int','STOK:string|25','CIKIS:string|25',
                    'MIKTAR:float','OZEL:string|50','GIRIS:int','PARTI:string|25']
        },
    },
    {
        ...
        select :    //KAYITLARIN GETİRİLECEĞİ SELECT SORGUSU
        {
            query:"SELECT  " + 
                  ...
                  "OZEL AS OZEL " + 
                  "FROM EMIR_HAREKETLERI WHERE TIP = 0 AND CINS = 3 AND DURUM IN(11,12)  " 
        },
        control:    //KAYITLAR KARŞI TARAFADA VAR İSE GÜNCELLEMESİ İÇİN KONTROL SORGUSU
        {
            query: "SELECT sth_evrakno_seri,sth_evrakno_sira FROM STOK_HAREKETLERI WHERE sth_evrakno_seri = @SERI and sth_evrakno_sira = @SIRA AND sth_tip = 0 AND sth_satirno =@SATIRNO ",
    ...
    },
    {
        ...
        insert : 
        {
           ...
           "([pl_DBCno] " +
            ...
            ")" +
            "UPDATE [dbo].[PARTILOT] SET " +
            "[pl_uretim_tar] =  pl_son_kullanim_tar - 365" +
            "WHERE [pl_partikodu] = @pl_partikodu",
